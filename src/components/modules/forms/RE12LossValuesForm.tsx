...
export default function RE12LossValuesForm({
  ...
  useEffect(() => {
    async function loadFireProtectionData() {
      try {
        ...
        if (fireProtectionInstance?.data?.fire_protection?.credible_to_reduce_nle) {
          const nleData = fireProtectionInstance.data.fire_protection.credible_to_reduce_nle;
          setFireProtectionCredible(nleData.credible || false);
          setFireProtectionBasis(nleData.basis || '');
        }
      } catch (err) {
  ...
  useEffect(() => {
    const wleTotal = (formData.loss_values.wle.property_loss || 0) + (formData.loss_values.wle.bi_loss || 0);
    const nleTotal = (formData.loss_values.nle.property_loss || 0) + (formData.loss_values.nle.bi_loss || 0);

    setFormData({
      ...formData,
      loss_values: {
        ...formData.loss_values,
        wle: { ...formData.loss_values.wle, total_wle: wleTotal },
        nle: { ...formData.loss_values.nle, total_nle: nleTotal },
      },
    });
  }, [
  ...
  const handleSave = async () => {
    if (formData.loss_values.nle.fire_protection_credited && !fireProtectionCredible) {
      alert('Fire protection cannot be credited in NLE: RE-06 Fire Protection module indicates it is not credible to reduce NLE.');
      return;
    }
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-lg border border-slate-200 p-6">
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Building & Contents</label>
              <input
                type="number"
        ...
        <div className="bg-white rounded-lg border border-slate-200 p-6">
          <h3 className="text-lg font-semibold text-slate-900 mb-4">Worst-Case Loss Expectancy (WLE)</h3>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Property Loss</label>
                <input
                  type="number"
                  ...
                  onChange={(e) => setFormData({
                    ...
                    loss_values: {
                      ...formData.loss_values,
                      wle: { ...formData.loss_values.wle, property_loss: e.target.value ? parseFloat(e.target.value) : null }
                    }
                  })}
                  className="w-full px-3 py-2 border border-slate-300 rounded-md text-sm"
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setFormData({
                    ...
                    loss_values: {
                      ...formData.loss_values,
                      wle: { ...formData.loss_values.wle, bi_loss: e.target.value ? parseFloat(e.target.value) : null }
                    }
                  })}
                  className="w-full px-3 py-2 border border-slate-300 rounded-md text-sm"
        ...
        <div className="bg-white rounded-lg border border-slate-200 p-6">
          ...
          <p className="text-sm text-slate-600 mb-4">
            Expected loss scenario with fire protection credited (if credible per RE-06).
          </p>
          ...
          {!fireProtectionCredible && (
            <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 mb-4 flex gap-3">
              ...
              <div>
                <p className="text-sm font-medium text-amber-900">Fire Protection Not Credible for NLE Reduction</p>
                <p className="text-sm text-amber-700 mt-1">
                  RE-06 Fire Protection indicates fire protection is not credible to materially reduce NLE.
          ...
          {fireProtectionCredible && (
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4 flex gap-3">
              ...
              <div>
                <p className="text-sm font-medium text-blue-900">Fire Protection Credible for NLE Reduction</p>
                <p className="text-sm text-blue-700 mt-1">
                  RE-06 Fire Protection indicates fire protection is credible to materially reduce NLE.
                  {fireProtectionBasis && ` Basis: ${fireProtectionBasis}`}
                </p>
              </div>
            </div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    if (e.target.checked && !fireProtectionCredible) {
                      alert('Cannot credit fire protection: RE-06 Fire Protection indicates it is not credible to reduce NLE.');
                      return;
                    }
                    setFormData({
                      ...formData,
                      loss_values: {
                        ...formData.loss_values,
                        nle: { ...formData.loss_values.nle, fire_protection_credited: e.target.checked }
                      }
                    });
                  }}
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setFormData({
                    ...
                    loss_values: {
                      ...formData.loss_values,
                      nle: { ...formData.loss_values.nle, property_loss: e.target.value ? parseFloat(e.target.value) : null }
                    }
                  })}
                  className="w-full px-3 py-2 border border-slate-300 rounded-md text-sm"
                  placeholder="Property damage value"
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setFormData({
                    ...
                    loss_values: {
                      ...formData.loss_values,
                      nle: { ...formData.loss_values.nle, bi_loss: e.target.value ? parseFloat(e.target.value) : null }
                    }
                  })}
                  className="w-full px-3 py-2 border border-slate-300 rounded-md text-sm"
                  placeholder="Business interruption loss"
        ...
        <div className="bg-white rounded-lg border border-slate-200 p-6">
          ...
          <div className="space-y-3">
            ...
            {formData.loss_values.wle.total_wle && formData.loss_values.nle.total_nle && (
              <div className="flex justify-between items-center py-2 bg-blue-50 px-4 rounded-lg">
                ...
                <span className="font-bold text-lg text-blue-900">
                  {Math.round((formData.loss_values.nle.total_nle / formData.loss_values.wle.total_wle) * 100)}%
                </span>
              </div>
            )}
          </div>
